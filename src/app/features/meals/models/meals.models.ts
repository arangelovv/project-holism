export interface Ingredient {
	ingridient: string; // Note: There's a typo in "ingredient" throughout the data
	grams: number;
}

export interface Meal {
	id: number;
	mealName: string;
	mealTag: "Breakfast" | "Lunch" | "Dinner";
	mealPrepTime: string;
	mealDifficulty?: "Easy" | "Medium" | "Hard"; // Optionala for now till we have data for all meals
	mealCalories: number;
	mealProtein: number;
	mealCarbs: number;
	mealFats: number;
	mealIngridients: Ingredient[];
	mealImg?: string; //Optional for now till we have data for all meals
	mealDescription?: string; // Optional for now till we have data for all meals
	instructions?: string[]; // Optional for now till we have data for all meals
}

export const meals: Meal[] = [
	{
		id: 1,
		mealName: "High-Protein Oatmeal",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealDifficulty: "Easy",
		mealCalories: 320,
		mealProtein: 25,
		mealCarbs: 40,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Oats",
				grams: 50,
			},
			{
				ingridient: "Whey Protein",
				grams: 30,
			},
			{
				ingridient: "Almond milk",
				grams: 200,
			},
			{
				ingridient: "Blueberries",
				grams: 50,
			},
		],
		mealImg:
			"https://images.ctfassets.net/092wi7ilt2cf/nXtqOslQpqabAnoLwBqi3/1bd8b436027d472e070e8cd211ae7265/High-Protein-Oatmeal-0528.jpg?fit=fill&w=1200&h=900",
		mealDescription:
			"A nutrient-rich breakfast bowl that combines the goodness of oats with protein powder and fresh fruits. Perfect for post-workout recovery or a filling breakfast that will keep you energized throughout the morning.",
		instructions: [
			"In a microwave-safe bowl, combine rolled oats with almond milk.",
			"Microwave for 2 minutes, stirring halfway through.",
			"Let it cool for 1 minute, then stir in the protein powder until well combined.",
			"Add chia seeds and stir well.",
			"Slice the banana and arrange on top.",
			"Add mixed berries.",
			"Drizzle with honey and serve immediately.",
		],
	},
	{
		id: 2,
		mealName: "Scrambled Eggs & Avocado Toast",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 350,
		mealProtein: 20,
		mealCarbs: 15,
		mealFats: 22,
		mealIngridients: [
			{
				ingridient: "Eggs",
				grams: 150,
			},
			{
				ingridient: "Bread",
				grams: 40,
			},
			{
				ingridient: "Avocado",
				grams: 50,
			},
		],
	},
	{
		id: 3,
		mealName: "Cottage Cheese & Pineapple Bowl",
		mealTag: "Breakfast",
		mealPrepTime: "3min",
		mealCalories: 230,
		mealProtein: 24,
		mealCarbs: 20,
		mealFats: 3,
		mealIngridients: [
			{
				ingridient: "Cottage cheese (low-fat)",
				grams: 200,
			},
			{
				ingridient: "Pineapple chunks",
				grams: 100,
			},
		],
	},
	{
		id: 4,
		mealName: "Spinach & Feta Egg White Omelette",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 180,
		mealProtein: 25,
		mealCarbs: 2,
		mealFats: 6,
		mealIngridients: [
			{
				ingridient: "Egg whites",
				grams: 200,
			},
			{
				ingridient: "Spinach",
				grams: 50,
			},
			{
				ingridient: "Feta cheese",
				grams: 30,
			},
		],
	},
	{
		id: 5,
		mealName: "Peanut Butter & Banana Smoothie",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 290,
		mealProtein: 27,
		mealCarbs: 35,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Banana",
				grams: 100,
			},
			{
				ingridient: "Peanut butter",
				grams: 10,
			},
			{
				ingridient: "Almond milk",
				grams: 200,
			},
			{
				ingridient: "Whey protein",
				grams: 30,
			},
		],
	},
	{
		id: 6,
		mealName: "Smoked Salmon on Rye Bread",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 200,
		mealProtein: 16,
		mealCarbs: 12,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Smoked salmon",
				grams: 50,
			},
			{
				ingridient: "Rye bread",
				grams: 40,
			},
			{
				ingridient: "Cream cheese (low-fat)",
				grams: 10,
			},
		],
	},
	{
		id: 7,
		mealName: "Protein Pancakes",
		mealTag: "Breakfast",
		mealPrepTime: "15min",
		mealCalories: 280,
		mealProtein: 30,
		mealCarbs: 25,
		mealFats: 4,
		mealIngridients: [
			{
				ingridient: "Oats",
				grams: 40,
			},
			{
				ingridient: "Egg whites",
				grams: 150,
			},
			{
				ingridient: "Whey protein",
				grams: 30,
			},
		],
	},
	{
		id: 8,
		mealName: "Greek Yogurt Parfait",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 210,
		mealProtein: 15,
		mealCarbs: 25,
		mealFats: 4,
		mealIngridients: [
			{
				ingridient: "Greek yogurt (non-fat)",
				grams: 150,
			},
			{
				ingridient: "Granola",
				grams: 30,
			},
			{
				ingridient: "Strawberries",
				grams: 50,
			},
		],
	},
	{
		id: 9,
		mealName: "Turkey & Egg Breakfast Wrap",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 290,
		mealProtein: 22,
		mealCarbs: 20,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Whole grain tortilla",
				grams: 50,
			},
			{
				ingridient: "Turkey slices",
				grams: 50,
			},
			{
				ingridient: "Scrambled eggs",
				grams: 100,
			},
		],
	},
	{
		id: 10,
		mealName: "Avocado & Egg Bowl",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 250,
		mealProtein: 15,
		mealCarbs: 5,
		mealFats: 18,
		mealIngridients: [
			{
				ingridient: "Hard-boiled eggs",
				grams: 100,
			},
			{
				ingridient: "Avocado",
				grams: 50,
			},
			{
				ingridient: "Cherry tomatoes",
				grams: 50,
			},
		],
	},
	{
		id: 11,
		mealName: "Apple & Almond Butter Toast",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 200,
		mealProtein: 5,
		mealCarbs: 28,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Apple slices",
				grams: 50,
			},
			{
				ingridient: "Almond butter",
				grams: 10,
			},
			{
				ingridient: "Whole grain bread",
				grams: 40,
			},
		],
	},
	{
		id: 12,
		mealName: "Chia Pudding",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 210,
		mealProtein: 10,
		mealCarbs: 12,
		mealFats: 12,
		mealIngridients: [
			{
				ingridient: "Chia seeds",
				grams: 30,
			},
			{
				ingridient: "Almond milk",
				grams: 200,
			},
			{
				ingridient: "Raspberries",
				grams: 50,
			},
		],
	},
	{
		id: 13,
		mealName: "Protein French Toast",
		mealTag: "Breakfast",
		mealPrepTime: "15min",
		mealCalories: 350,
		mealProtein: 30,
		mealCarbs: 35,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Whole grain bread",
				grams: 80,
			},
			{
				ingridient: "Eggs",
				grams: 100,
			},
			{
				ingridient: "Whey protein",
				grams: 20,
			},
		],
	},
	{
		id: 14,
		mealName: "Tuna & Avocado Crackers",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 220,
		mealProtein: 18,
		mealCarbs: 18,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Whole grain crackers",
				grams: 40,
			},
			{
				ingridient: "Tuna (in water)",
				grams: 50,
			},
			{
				ingridient: "Avocado",
				grams: 30,
			},
		],
	},
	{
		id: 15,
		mealName: "High-Protein Smoothie Bowl",
		mealTag: "Breakfast",
		mealPrepTime: "5min",
		mealCalories: 480,
		mealProtein: 33,
		mealCarbs: 62,
		mealFats: 14,
		mealIngridients: [
			{
				ingridient: "Frozen berries",
				grams: 100,
			},
			{
				ingridient: "Whey protein",
				grams: 30,
			},
			{
				ingridient: "Almond milk",
				grams: 200,
			},
			{
				ingridient: "Granola",
				grams: 50,
			},
		],
	},
	{
		id: 16,
		mealName: "Egg & Spinach Breakfast Muffins",
		mealTag: "Breakfast",
		mealPrepTime: "20min",
		mealCalories: 230,
		mealProtein: 28,
		mealCarbs: 2,
		mealFats: 12,
		mealIngridients: [
			{
				ingridient: "Eggs",
				grams: 200,
			},
			{
				ingridient: "Spinach",
				grams: 50,
			},
			{
				ingridient: "Feta cheese",
				grams: 30,
			},
		],
	},
	{
		id: 17,
		mealName: "Protein Overnight Oats",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 300,
		mealProtein: 25,
		mealCarbs: 35,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Oats",
				grams: 50,
			},
			{
				ingridient: "Greek yogurt",
				grams: 100,
			},
			{
				ingridient: "Whey protein",
				grams: 20,
			},
			{
				ingridient: "Almond milk",
				grams: 150,
			},
		],
	},
	{
		id: 18,
		mealName: "Turkey Bacon & Egg Sandwich",
		mealTag: "Breakfast",
		mealPrepTime: "10min",
		mealCalories: 250,
		mealProtein: 18,
		mealCarbs: 20,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Whole grain English muffin",
				grams: 60,
			},
			{
				ingridient: "Turkey bacon",
				grams: 30,
			},
			{
				ingridient: "Scrambled egg",
				grams: 50,
			},
		],
	},
	{
		id: 19,
		mealName: "Tofu Scramble & Veggies",
		mealTag: "Breakfast",
		mealPrepTime: "15min",
		mealCalories: 180,
		mealProtein: 20,
		mealCarbs: 8,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Tofu (firm)",
				grams: 100,
			},
			{
				ingridient: "Bell peppers",
				grams: 50,
			},
			{
				ingridient: "Spinach",
				grams: 50,
			},
		],
	},
	{
		id: 20,
		mealName: "Berry Protein Waffles",
		mealTag: "Breakfast",
		mealPrepTime: "15min",
		mealCalories: 300,
		mealProtein: 28,
		mealCarbs: 35,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Oat flour",
				grams: 50,
			},
			{
				ingridient: "Whey protein",
				grams: 30,
			},
			{
				ingridient: "Almond milk",
				grams: 150,
			},
			{
				ingridient: "Mixed berries",
				grams: 50,
			},
		],
	},
	{
		id: 21,
		mealName: "Grilled Chicken Salad",
		mealTag: "Lunch",
		mealPrepTime: "15min",
		mealCalories: 280,
		mealProtein: 35,
		mealCarbs: 5,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Mixed greens",
				grams: 100,
			},
			{
				ingridient: "Cherry tomatoes",
				grams: 50,
			},
			{
				ingridient: "Olive oil (dressing)",
				grams: 10,
			},
		],
	},
	{
		id: 22,
		mealName: "Quinoa & Salmon Bowl",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 400,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 15,
		mealIngridients: [
			{
				ingridient: "Quinoa (cooked)",
				grams: 100,
			},
			{
				ingridient: "Salmon fillet",
				grams: 150,
			},
			{
				ingridient: "Steamed broccoli",
				grams: 100,
			},
		],
	},
	{
		id: 23,
		mealName: "Turkey & Sweet Potato",
		mealTag: "Lunch",
		mealPrepTime: "25min",
		mealCalories: 350,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Ground turkey",
				grams: 150,
			},
			{
				ingridient: "Sweet potato",
				grams: 150,
			},
			{
				ingridient: "Steamed green beans",
				grams: 100,
			},
		],
	},
	{
		id: 24,
		mealName: "Tuna Salad Wrap",
		mealTag: "Lunch",
		mealPrepTime: "10min",
		mealCalories: 280,
		mealProtein: 35,
		mealCarbs: 20,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Tuna (in water)",
				grams: 100,
			},
			{
				ingridient: "Whole grain tortilla",
				grams: 50,
			},
			{
				ingridient: "Lettuce",
				grams: 50,
			},
			{
				ingridient: "Low-fat mayonnaise",
				grams: 10,
			},
		],
	},
	{
		id: 25,
		mealName: "Chicken & Brown Rice Bowl",
		mealTag: "Lunch",
		mealPrepTime: "25min",
		mealCalories: 360,
		mealProtein: 35,
		mealCarbs: 35,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Grilled chicken",
				grams: 150,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
			{
				ingridient: "Roasted carrots",
				grams: 100,
			},
		],
	},
	{
		id: 26,
		mealName: "Turkey Burger Bowl",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 330,
		mealProtein: 30,
		mealCarbs: 10,
		mealFats: 20,
		mealIngridients: [
			{
				ingridient: "Turkey patty (lean)",
				grams: 150,
			},
			{
				ingridient: "Mixed greens",
				grams: 100,
			},
			{
				ingridient: "Avocado",
				grams: 30,
			},
			{
				ingridient: "Olive oil (dressing)",
				grams: 5,
			},
		],
	},
	{
		id: 27,
		mealName: "Grilled Shrimp & Couscous",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 320,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Shrimp",
				grams: 150,
			},
			{
				ingridient: "Couscous (cooked)",
				grams: 100,
			},
			{
				ingridient: "Steamed zucchini",
				grams: 100,
			},
		],
	},
	{
		id: 28,
		mealName: "Teriyaki Chicken Bowl",
		mealTag: "Lunch",
		mealPrepTime: "25min",
		mealCalories: 380,
		mealProtein: 35,
		mealCarbs: 40,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Jasmine rice (cooked)",
				grams: 100,
			},
			{
				ingridient: "Teriyaki sauce",
				grams: 10,
			},
			{
				ingridient: "Steamed broccoli",
				grams: 100,
			},
		],
	},
	{
		id: 29,
		mealName: "Spicy Chicken & Veggie Stir-Fry",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 350,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Bell peppers",
				grams: 100,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
			{
				ingridient: "Soy sauce",
				grams: 10,
			},
		],
	},
	{
		id: 30,
		mealName: "Turkey Meatball Zoodles",
		mealTag: "Lunch",
		mealPrepTime: "30min",
		mealCalories: 250,
		mealProtein: 35,
		mealCarbs: 10,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Ground turkey",
				grams: 150,
			},
			{
				ingridient: "Zucchini noodles",
				grams: 200,
			},
			{
				ingridient: "Marinara sauce",
				grams: 50,
			},
		],
	},
	{
		id: 31,
		mealName: "Lentil & Chicken Soup",
		mealTag: "Lunch",
		mealPrepTime: "30min",
		mealCalories: 290,
		mealProtein: 30,
		mealCarbs: 30,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Lentils (cooked)",
				grams: 100,
			},
			{
				ingridient: "Chicken breast (shredded)",
				grams: 100,
			},
			{
				ingridient: "Carrots",
				grams: 50,
			},
			{
				ingridient: "Chicken broth",
				grams: 250,
			},
		],
	},
	{
		id: 32,
		mealName: "Beef & Quinoa Bowl",
		mealTag: "Lunch",
		mealPrepTime: "25min",
		mealCalories: 450,
		mealProtein: 33,
		mealCarbs: 35,
		mealFats: 21,
		mealIngridients: [
			{
				ingridient: "Lean ground beef (raw)",
				grams: 150,
			},
			{
				ingridient: "Quinoa (raw)",
				grams: 40,
			},
			{
				ingridient: "Roasted cauliflower",
				grams: 100,
			},
		],
	},
	{
		id: 33,
		mealName: "Salmon & Asparagus Plate",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 400,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 15,
		mealIngridients: [
			{
				ingridient: "Salmon fillet",
				grams: 150,
			},
			{
				ingridient: "Steamed asparagus",
				grams: 100,
			},
			{
				ingridient: "Mashed sweet potato (cooked)",
				grams: 100,
			},
		],
	},
	{
		id: 34,
		mealName: "Grilled Chicken & Hummus Wrap",
		mealTag: "Lunch",
		mealPrepTime: "10min",
		mealCalories: 320,
		mealProtein: 30,
		mealCarbs: 25,
		mealFats: 9,
		mealIngridients: [
			{
				ingridient: "Grilled chicken",
				grams: 100,
			},
			{
				ingridient: "Whole grain tortilla",
				grams: 50,
			},
			{
				ingridient: "Hummus",
				grams: 20,
			},
			{
				ingridient: "Spinach",
				grams: 50,
			},
		],
	},
	{
		id: 35,
		mealName: "Cod & Vegetable Bowl",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 330,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Cod fillet",
				grams: 150,
			},
			{
				ingridient: "Steamed broccoli",
				grams: 100,
			},
			{
				ingridient: "Quinoa (cooked)",
				grams: 100,
			},
		],
	},
	{
		id: 36,
		mealName: "Baked Chicken Thighs & Rice",
		mealTag: "Lunch",
		mealPrepTime: "40min",
		mealCalories: 400,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 12,
		mealIngridients: [
			{
				ingridient: "Chicken thighs (boneless, skinless)",
				grams: 150,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
			{
				ingridient: "Roasted Brussels sprouts",
				grams: 100,
			},
		],
	},
	{
		id: 37,
		mealName: "Vegan Tofu & Quinoa Bowl",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 320,
		mealProtein: 30,
		mealCarbs: 25,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Tofu (firm)",
				grams: 150,
			},
			{
				ingridient: "Quinoa (cooked)",
				grams: 100,
			},
			{
				ingridient: "Steamed kale",
				grams: 100,
			},
		],
	},
	{
		id: 38,
		mealName: "Grilled Steak & Sweet Potato",
		mealTag: "Lunch",
		mealPrepTime: "30min",
		mealCalories: 380,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Lean steak",
				grams: 150,
			},
			{
				ingridient: "Sweet potato",
				grams: 150,
			},
			{
				ingridient: "Green beans",
				grams: 100,
			},
		],
	},
	{
		id: 39,
		mealName: "Black Bean & Chicken Burrito",
		mealTag: "Lunch",
		mealPrepTime: "20min",
		mealCalories: 320,
		mealProtein: 30,
		mealCarbs: 35,
		mealFats: 6,
		mealIngridients: [
			{
				ingridient: "Black beans",
				grams: 50,
			},
			{
				ingridient: "Grilled chicken",
				grams: 100,
			},
			{
				ingridient: "Whole grain tortilla",
				grams: 50,
			},
			{
				ingridient: "Salsa",
				grams: 20,
			},
		],
	},
	{
		id: 40,
		mealName: "Turkey & Rice Stuffed Peppers",
		mealTag: "Lunch",
		mealPrepTime: "40min",
		mealCalories: 300,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 6,
		mealIngridients: [
			{
				ingridient: "Ground turkey",
				grams: 150,
			},
			{
				ingridient: "Cooked rice",
				grams: 50,
			},
			{
				ingridient: "Bell peppers",
				grams: 200,
			},
			{
				ingridient: "Tomato sauce",
				grams: 30,
			},
		],
	},
	{
		id: 41,
		mealName: "Turkey Stir-Fry",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 425,
		mealProtein: 24,
		mealCarbs: 39,
		mealFats: 18,
		mealIngridients: [
			{
				ingridient: "Ground turkey (raw)",
				grams: 150,
			},
			{
				ingridient: "Bell peppers",
				grams: 100,
			},
			{
				ingridient: "Olive oil",
				grams: 5,
			},
			{
				ingridient: "Brown rice (raw)",
				grams: 40,
			},
		],
	},
	{
		id: 42,
		mealName: "Shrimp & Zoodles",
		mealTag: "Dinner",
		mealPrepTime: "15min",
		mealCalories: 280,
		mealProtein: 40,
		mealCarbs: 10,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Shrimp",
				grams: 150,
			},
			{
				ingridient: "Zucchini noodles",
				grams: 200,
			},
			{
				ingridient: "Garlic",
				grams: 2,
			},
			{
				ingridient: "Olive oil",
				grams: 5,
			},
		],
	},
	{
		id: 43,
		mealName: "Grilled Chicken & Veggie Skewers",
		mealTag: "Dinner",
		mealPrepTime: "25min",
		mealCalories: 320,
		mealProtein: 35,
		mealCarbs: 10,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Bell peppers",
				grams: 50,
			},
			{
				ingridient: "Zucchini",
				grams: 50,
			},
			{
				ingridient: "Olive oil",
				grams: 5,
			},
		],
	},
	{
		id: 44,
		mealName: "Baked Salmon & Quinoa",
		mealTag: "Dinner",
		mealPrepTime: "25min",
		mealCalories: 400,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 15,
		mealIngridients: [
			{
				ingridient: "Salmon fillet",
				grams: 150,
			},
			{
				ingridient: "Quinoa (cooked)",
				grams: 100,
			},
			{
				ingridient: "Steamed asparagus",
				grams: 100,
			},
		],
	},
	{
		id: 45,
		mealName: "Beef Stir-Fry with Broccoli",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 380,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Lean beef strips",
				grams: 150,
			},
			{
				ingridient: "Broccoli",
				grams: 100,
			},
			{
				ingridient: "Soy sauce",
				grams: 10,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
		],
	},
	{
		id: 46,
		mealName: "Turkey Meatloaf & Sweet Potato",
		mealTag: "Dinner",
		mealPrepTime: "40min",
		mealCalories: 350,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Ground turkey",
				grams: 150,
			},
			{
				ingridient: "Sweet potato (baked)",
				grams: 150,
			},
			{
				ingridient: "Steamed green beans",
				grams: 100,
			},
		],
	},
	{
		id: 47,
		mealName: "Lemon Garlic Cod",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 300,
		mealProtein: 35,
		mealCarbs: 20,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Cod fillet",
				grams: 150,
			},
			{
				ingridient: "Steamed spinach",
				grams: 100,
			},
			{
				ingridient: "Mashed sweet potato",
				grams: 100,
			},
		],
	},
	{
		id: 48,
		mealName: "BBQ Chicken & Grilled Vegetables",
		mealTag: "Dinner",
		mealPrepTime: "25min",
		mealCalories: 320,
		mealProtein: 35,
		mealCarbs: 15,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Zucchini",
				grams: 50,
			},
			{
				ingridient: "Bell peppers",
				grams: 50,
			},
			{
				ingridient: "BBQ sauce",
				grams: 10,
			},
		],
	},
	{
		id: 49,
		mealName: "Vegan Lentil Curry",
		mealTag: "Dinner",
		mealPrepTime: "30min",
		mealCalories: 280,
		mealProtein: 20,
		mealCarbs: 25,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Lentils (cooked)",
				grams: 100,
			},
			{
				ingridient: "Coconut milk (light)",
				grams: 50,
			},
			{
				ingridient: "Curry powder",
				grams: 5,
			},
			{
				ingridient: "Steamed broccoli",
				grams: 100,
			},
		],
	},
	{
		id: 50,
		mealName: "Grilled Tuna Steak & Couscous",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 330,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Tuna steak",
				grams: 150,
			},
			{
				ingridient: "Couscous (cooked)",
				grams: 100,
			},
			{
				ingridient: "Roasted zucchini",
				grams: 100,
			},
		],
	},
	{
		id: 51,
		mealName: "Stuffed Bell Peppers",
		mealTag: "Dinner",
		mealPrepTime: "40min",
		mealCalories: 300,
		mealProtein: 35,
		mealCarbs: 20,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Ground turkey",
				grams: 150,
			},
			{
				ingridient: "Cooked quinoa",
				grams: 50,
			},
			{
				ingridient: "Bell peppers",
				grams: 200,
			},
			{
				ingridient: "Tomato sauce",
				grams: 30,
			},
		],
	},
	{
		id: 52,
		mealName: "Chicken & Spinach Alfredo",
		mealTag: "Dinner",
		mealPrepTime: "25min",
		mealCalories: 290,
		mealProtein: 35,
		mealCarbs: 8,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Zucchini noodles",
				grams: 200,
			},
			{
				ingridient: "Low-fat Alfredo sauce",
				grams: 50,
			},
		],
	},
	{
		id: 53,
		mealName: "Baked Tilapia with Brown Rice",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 320,
		mealProtein: 35,
		mealCarbs: 30,
		mealFats: 5,
		mealIngridients: [
			{
				ingridient: "Tilapia fillet",
				grams: 150,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
			{
				ingridient: "Steamed broccoli",
				grams: 100,
			},
		],
	},
	{
		id: 54,
		mealName: "Turkey Chili",
		mealTag: "Dinner",
		mealPrepTime: "35min",
		mealCalories: 330,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Ground turkey (raw)",
				grams: 150,
			},
			{
				ingridient: "Diced tomatoes",
				grams: 100,
			},
			{
				ingridient: "Kidney beans (cooked)",
				grams: 50,
			},
			{
				ingridient: "Onion",
				grams: 30,
			},
		],
	},
	{
		id: 55,
		mealName: "Grilled Chicken Caesar Salad",
		mealTag: "Dinner",
		mealPrepTime: "15min",
		mealCalories: 320,
		mealProtein: 35,
		mealCarbs: 5,
		mealFats: 12,
		mealIngridients: [
			{
				ingridient: "Grilled chicken",
				grams: 150,
			},
			{
				ingridient: "Romaine lettuce",
				grams: 100,
			},
			{
				ingridient: "Caesar dressing (low-fat)",
				grams: 10,
			},
			{
				ingridient: "Parmesan cheese",
				grams: 10,
			},
		],
	},
	{
		id: 56,
		mealName: "Beef & Sweet Potato Hash",
		mealTag: "Dinner",
		mealPrepTime: "25min",
		mealCalories: 350,
		mealProtein: 35,
		mealCarbs: 25,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Lean ground beef",
				grams: 150,
			},
			{
				ingridient: "Sweet potato (diced)",
				grams: 150,
			},
			{
				ingridient: "Onion",
				grams: 30,
			},
		],
	},
	{
		id: 57,
		mealName: "Grilled Swordfish & Vegetables",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 310,
		mealProtein: 35,
		mealCarbs: 10,
		mealFats: 10,
		mealIngridients: [
			{
				ingridient: "Swordfish steak",
				grams: 150,
			},
			{
				ingridient: "Roasted Brussels sprouts",
				grams: 100,
			},
			{
				ingridient: "Mashed cauliflower",
				grams: 100,
			},
		],
	},
	{
		id: 58,
		mealName: "Baked Chicken Parmesan",
		mealTag: "Dinner",
		mealPrepTime: "30min",
		mealCalories: 330,
		mealProtein: 35,
		mealCarbs: 10,
		mealFats: 12,
		mealIngridients: [
			{
				ingridient: "Chicken breast",
				grams: 150,
			},
			{
				ingridient: "Marinara sauce",
				grams: 50,
			},
			{
				ingridient: "Mozzarella cheese (low-fat)",
				grams: 20,
			},
			{
				ingridient: "Zucchini noodles",
				grams: 200,
			},
		],
	},
	{
		id: 59,
		mealName: "Shrimp & Cauliflower Rice",
		mealTag: "Dinner",
		mealPrepTime: "15min",
		mealCalories: 280,
		mealProtein: 40,
		mealCarbs: 10,
		mealFats: 8,
		mealIngridients: [
			{
				ingridient: "Shrimp",
				grams: 150,
			},
			{
				ingridient: "Cauliflower rice",
				grams: 200,
			},
			{
				ingridient: "Olive oil",
				grams: 5,
			},
			{
				ingridient: "Soy sauce",
				grams: 10,
			},
		],
	},
	{
		id: 60,
		mealName: "Vegan Tofu Stir-Fry",
		mealTag: "Dinner",
		mealPrepTime: "20min",
		mealCalories: 320,
		mealProtein: 30,
		mealCarbs: 30,
		mealFats: 7,
		mealIngridients: [
			{
				ingridient: "Tofu (firm)",
				grams: 150,
			},
			{
				ingridient: "Mixed vegetables (carrots, broccoli, bell peppers)",
				grams: 100,
			},
			{
				ingridient: "Soy sauce",
				grams: 10,
			},
			{
				ingridient: "Brown rice (cooked)",
				grams: 100,
			},
		],
	},
];
